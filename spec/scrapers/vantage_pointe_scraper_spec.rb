...
RSpec.describe VantagePointeScraper, type: :model do
  describe ".scrape" do
    Given(:sample_page) { File.open('./spec/pages/vantage_pointe.html', 'r') }
    Given { stub_request(:get, VantagePointeScraper::LISTINGS_URL).to_return(body: sample_page) }
    When(:result) { VantagePointeScraper.scrape }
